...
class FixedRangeStrategy(TradeStrategy):
    def run_loop(self) -> None:
        mark_price = self.repo.get_mark_price(symbol=Settings().file_input.symbol).markPrice
        position_risk = self.repo.get_position_risk(symbol=Settings().file_input.symbol)
        ...
        (price_sell_max, price_sell_min, price_buy_max, price_buy_min) = get_grid_maxs_and_mins(
            ...
            price_sell_max_mult=Settings().file_input.price_sell_max_mult,
            price_sell_min_mult=Settings().file_input.price_sell_min_mult,
            price_buy_max_mult=Settings().file_input.price_buy_max_mult,
        ...
        if position_amount < max_mm_position or not Settings().file_input.market_making:
            buy_orders_quantities_and_prices = get_buy_orders_quantities_and_prices(
                ...
                side=Settings().file_input.position_side,
                precision=3,
                ...
                mm_buy_quantity=Settings().file_input.mm_buy_quantity,
            )
            side_open = Side.BUY if Settings().file_input.position_side == PositionSide.LONG else Side.SELL
        ...
        sell_orders_quantities_and_prices = get_sell_orders_quantities_and_prices(
            ...
            amount_spacing=AmountSpacing.GEOMETRIC,
            market_making=Settings().file_input.market_making,
            mm_sell_quantity=Settings().file_input.mm_sell_quantity,
        )
        side_close = Side.SELL if Settings().file_input.position_side == PositionSide.LONG else Side.BUY
