...
class AllPriceMatchQueueStrategy(TradeStrategy):
    def run_loop(self):
        ...
        buy_orders = create_all_queue_price_match_orders(
            symbol=file_input.symbol, side=Side.BUY, position_side=position_side, quantity=min(buy_order_amount, 1000.0)
        )
        sell_orders = create_all_queue_price_match_orders(
            symbol=symbol, side=Side.SELL, position_side=position_side, quantity=min(sell_order_amount, 1000.0)
        )
