...
def log_client_error(error: ClientError):
    logging.error(f"{error.status_code=}, {error.error_code=}, {error.error_message=}")


def cancel_all_orders_request(symbol, recvWindow: int = 2000):
    response = {}
    try:
...
def new_batch_order_request(params):
    response = {}
    try:
...
def get_position_risk_request(symbol: str):
    response = {}
    try:
...
def get_leverage_request(symbol: str):
    response = {}
    try:
...
def get_mark_price_request(symbol: str):
    response = {}
    try:
...
def close_listen_key_request(listenKey: str):
    response = {}
    try:
...
def get_open_orders_request(symbol: str):
    response = {}
    try:
