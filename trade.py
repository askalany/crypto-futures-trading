...
def trade(
    ...
    if strategy is Strategy.FIXED_RANGE:
        for order in trade_fixed_range(
            ...
            sell_amount=get_hedge_position_amount(symbol),
...
def trade_fixed_range(
    ...
    buy_high_price = mark_price * (1.0 - 0.001)
    buy_low_price = mark_price * (1.0 - 0.2)
    sell_high_price = mark_price * (1.0 + 0.2)
    sell_low_price = mark_price * (1.0 + 0.001)
...
def get_max_buy_amount(symbol: TickerSymbol):
    return (get_leverage(symbol) * get_available_balance()) / get_mark_price(symbol)
